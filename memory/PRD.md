...
### Phase 8 — Non-destructive Force Re-Generate + fixture-sync enrichment (2026-05-11)
- **CRITICAL UX FIX — Force Re-Generate no longer wipes a working slip**: `slip_generate(force=true)` in `server.py` previously called `picks_col.delete_many` BEFORE knowing if the new run produced any picks. If the new run yielded 0 (e.g. Odds API rate-limited, cache miss), the user was left with nothing. Now it only deletes existing picks when `new picks >= 1`. Sets `job.kept_old=true` and the admin toast says "Re-run produced 0 picks — KEPT existing slip intact".
- **CRITICAL DATA FIX — fixture-sync picks now ship with real data_richness**: `fixture_sync_service.run_ai_for_new_odds` rebuilt Fixture from cached odds payload without enrichment, so `data_richness=0.0` and the slip-quality gate (≥40%) suppressed valid picks. Now calls `_enrich_one(db, fx)` (parallel, capped at 6 concurrent) before `run_ensemble` so form / injuries / h2h populate properly. This was the root cause of the production "2 READY fixtures but 'awaiting real data' on dashboard" bug.
- Verified: 39 backend pytest pass total (`test_phase4/5/6/7/8`, full regression green).
