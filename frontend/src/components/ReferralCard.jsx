...
export default function ReferralCard() {
  ...
  return (
    <div className="co-soft-band rounded-[8px] p-5" data-testid="referral-card">
      <div className="flex items-center gap-2 mb-3">
        <span className="w-9 h-9 rounded-[8px] bg-[#00ff66]/12 grid place-items-center">
          <Gift className="w-4 h-4 text-[#00ff66]" />
        </span>
        <h3 className="font-heading font-bold text-base">REFER & EARN</h3>
        <span className="ml-auto font-mono text-[10px] uppercase tracking-widest text-[#667482] inline-flex items-center gap-1">
          <Users className="w-3 h-3" />
          <span data-testid="ref-count">{data.count}</span> referred
        </span>
      </div>

      <p className="text-xs text-[#aeb8c2] leading-relaxed mb-4">
      ...
      <div className="space-y-2">
        {editing ? (
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 bg-[#0a0a0a] border border-[#00ff66] focus:border-[#f5f5f5] outline-none font-mono px-3 py-3 text-base tracking-[0.2em] font-bold uppercase rounded-[6px]"
            />
            <button onClick={saveCode} disabled={saving} data-testid="ref-edit-save"
                    className="co-primary-action rounded-[6px] font-mono text-[11px] uppercase tracking-widest px-4 py-3 disabled:opacity-50 inline-flex items-center gap-2 min-h-[44px]">
              {saving ? "Saving…" : "Save"}
            </button>
            <button onClick={cancelEdit} disabled={saving} data-testid="ref-edit-cancel"
                    className="co-secondary-action rounded-[6px] font-mono text-[11px] uppercase tracking-widest px-3 py-3 inline-flex items-center min-h-[44px]">
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            <div className="flex-1 bg-[#0a0a0a] border border-white/10 rounded-[6px] px-3 py-3 font-mono text-base tracking-[0.2em] font-bold" data-testid="ref-code">
              {data.code}
            </div>
            <button onClick={startEdit} data-testid="ref-edit-btn"
                    className="co-secondary-action rounded-[6px] font-mono text-[11px] uppercase tracking-widest px-3 py-3 inline-flex items-center gap-2 min-h-[44px]"
                    title="Customize your code">
              <Pencil className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => copy(data.code, "code")} data-testid="ref-copy-code"
                    className="co-secondary-action rounded-[6px] font-mono text-[11px] uppercase tracking-widest px-4 py-3 inline-flex items-center gap-2 min-h-[44px]">
        ...
        {editing && (
          <p className="text-[10px] font-mono text-[#525252] leading-relaxed">
            4-20 letters and numbers · must contain at least one letter · reserved words like ADMIN are blocked
          </p>
        )}
        <div className="flex items-center gap-2">
          <div className="flex-1 bg-[#0a0a0a] border border-white/10 rounded-[6px] px-3 py-3 font-mono text-xs text-[#aeb8c2] truncate" data-testid="ref-link">
            {data.share_link}
          </div>
          <button onClick={() => copy(data.share_link, "link")} data-testid="ref-copy-link"
                  className="co-primary-action rounded-[6px] font-mono text-[11px] uppercase tracking-widest px-4 py-3 inline-flex items-center gap-2 min-h-[44px]">
